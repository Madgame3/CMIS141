...
*Purpose: Display State Bird and flower of Each state
*/

//Import each required Java class
import java.util.Scanner;

public class StateLookup_test {
        ...
        String inputState = "";
        //I use the flag check if any results were found
        boolean flag = false ;
        while(inputState.compareToIgnoreCase("none")!=0){
            ...
            if(inputState.compareToIgnoreCase("none")==0){
                System.exit(0);
            }
            for(int i=0; i<50; i++){
                //System.out.println(stateInfo[i][0]);
                if(inputState.compareToIgnoreCase(stateInfo[i][0])==0){
                   System.out.println("------------------------------------------");
            ...
            if(flag == false){
                System.err.println("Error: No State Info, check your spelling");
            }
        }
    } 
}
